...
export default function CommandPalette(): JSX.Element {
  ...
  const base = useMemo<Cmd[]>(() => {
    ...
    // Not gated on projectPath: a folderless workspace can still run terminals
    // (they start in the home directory), and hiding these left "New workspace"
    // with no launch path at all.
    if (!full) {
    ...
    // Naturally empty when the workspace has no agents yet.
    agents.forEach((a) =>
